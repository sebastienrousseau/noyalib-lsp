...
- Tracks `noyalib` 0.0.51 under the exact lockstep pin and aligns the VS Code
...
- Tracks `noyalib` 0.0.50 under the exact lockstep pin and aligns the VS Code
  extension metadata with the release.
...
- Tracks `noyalib` 0.0.49 under the exact lockstep pin and aligns the VS Code
  extension metadata with the release.
...
- Tracks `noyalib` 0.0.48 under the exact lockstep pin and aligns the VS Code
  extension metadata with the release.
...
- Tracks `noyalib` 0.0.47 under the exact lockstep pin and aligns the VS Code
  extension metadata with the release.
...
- Tracks `noyalib` 0.0.44. The core release fixes a serializer defect
  ...
  else — the formatter writes through noyalib, so this matters here. The
  fix arrives via the version pin; no source change.
