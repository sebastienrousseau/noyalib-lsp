...
#[test]
fn diagnostics_agree_with_the_suite_on_every_case() {
    let Some(dir) = suite_dir() else { return };
    let cases = load_cases(&dir);
    let mut failures = Vec::new();
    for case in &cases {
        let diags = noyalib_lsp::diagnostics::collect(&case.yaml);
        let has_error = diags.iter().any(|d| d["severity"].as_i64() == Some(1));
        if has_error != case.fail {
            failures.push(format!(
                "{}: error_diagnostic={has_error} expected_fail={}",
                case.id, case.fail
            ));
        }
    }
    eprintln!(
        "yaml-test-suite (lsp diagnostics): {} passed, {} failed",
